...
const CartPage = () => {
  ...
  const [couponSummary, setCouponSummary] = useState({
    discountAmount: Number(localStorage.getItem("couponDiscount") || 0),
    isValid: !!localStorage.getItem("couponCode"),
    message: "",
  });
  ...
  const goMarketItems = useMemo(
    () =>
      (context?.cartData || []).filter((item) => {
        const source = String(item?.source || "").toLowerCase();
        const brand = String(item?.brand || "").toLowerCase();
        const isGoMarketSeller =
          item?.sellerId?.storeProfile?.marketId != null ||
  ...
  const nonGoMarketItems = useMemo(
    () => (context?.cartData || []).filter((item) => !goMarketItems.includes(item)),
    [context?.cartData, goMarketItems]
  );
  ...
  const freeByRule =
    commerceSettings.freeShippingAbove > 0 && baseAfterDiscount >= commerceSettings.freeShippingAbove;

  const goMarketShippingFee = hasGoMarketItems ? Math.round(Number(commerceSettings.goMarketShippingFee || 0)) : 0;
  const goMarketBaseDeliveryFee =
    hasGoMarketItems && !freeByRule ? Math.round(Number(commerceSettings.goMarketBaseDeliveryFee || 0)) : 0;
  const goMarketDistanceDeliveryFee =
    hasGoMarketItems && !freeByRule
        ...
        <div className="rightPart w-full lg:w-[30%]">
          <div className="shadow-md rounded-md bg-white p-5 sticky top-[155px]">
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-start gap-2.5">
                                <div
                                  className={`flex-shrink-0 px-2 py-1 border border-dashed rounded text-[11px] font-bold ${
                                    isDisabled
                                ...
                                <div className="flex-1 min-w-0">
                                  <p
                                    className={`text-[11px] font-[700] ${
                                      isDisabled ? "text-gray-400" : "text-green-700"
                                    }`}
                                  >
                                    {discountText}
                                  </p>
                  ...
                  {(couponSummary.message || couponMessage) && (
                    <p
                      className={`flex items-center gap-1.5 text-[12px] mt-2 ${
                        couponSummary.isValid ? "text-green-600" : "text-red-500"
                      }`}
                    >
            ...
            {couponSummary.discountAmount > 0 && (
              <p className="flex justify-between">
                <span>Discount</span>
                <span className="text-green-600">-₹{couponSummary.discountAmount}</span>
              </p>
            )}
            ...
            <p className="flex justify-between">
              <span>
                Delivery Fee
                {hasGoMarketItems && goMarketDistanceKm > 0 ? ` (${goMarketDistanceKm.toFixed(1)} km)` : ""}
              </span>
