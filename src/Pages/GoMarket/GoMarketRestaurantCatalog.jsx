...
export default function GoMarketRestaurantCatalog({ restaurantId, searchMode = false, initialQuery = "", onRestaurant }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
        <select className="gmp-select" style={{ paddingLeft: 12 }} value={menuId} onChange={(e) => setMenuId(e.target.value)}><option value="">All menus</option>{(filterMeta?.menus || []).map((m) => <option key={m._id} value={m._id}>{m.name}</option>)}</select>
      </div>
